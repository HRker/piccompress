...
class ImageCompressorGUI:
    def __init__(self, root):
        ...
        self.root.geometry("600x600")  # 加大視窗以容納說明文字
...
"""
        ...
        ttk.Label(main_frame, text="選擇資料夾:").grid(row=1, column=0, sticky=tk.W, pady=5)
        ttk.Entry(main_frame, textvariable=self.folder_path, width=50).grid(row=1, column=1, pady=5)
        ttk.Button(main_frame, text="瀏覽", command=self.browse_folder).grid(row=1, column=2, padx=5, pady=5)

        # 品質滑動條
        ttk.Label(main_frame, text="壓縮品質(1-95):").grid(row=2, column=0, sticky=tk.W, pady=5)
        ...
        self.quality_slider.grid(row=2, column=1, sticky=tk.W, pady=5)
        ttk.Label(main_frame, textvariable=self.quality_var).grid(row=2, column=2, pady=5)

        # 開始壓縮按鈕
        ttk.Button(main_frame, text="開始壓縮", command=self.start_compression).grid(row=3, column=0,
