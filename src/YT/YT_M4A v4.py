...
class VideoTranscriber:
    ...
    def update_progress_time(self):
        """更新處理時間"""
        while self.is_processing and self.start_time:
            try:
                elapsed = time.time() - self.start_time
                self.log_progress(f"已處理 {elapsed / 60:.1f} 分鐘...")
    ...
    def process_mp4(self):
        ...
        try:
            ...
            # 預估時間
            estimated_time = file_size * 0.5  # 粗略估計：每 MB 需要 0.5 分鐘
            self.log_progress(f"預估處理時間: 約 {estimated_time:.1f} 分鐘")
            ...
            self.log_progress(f"\n處理完成！總共耗時: {elapsed_time / 60:.1f} 分鐘")
    ...
    def process_video(self):
        ...
        try:
            ...
            if file_size > 50:  # 如果大於 50MB
                self.log_progress("檔案較大，將進行分段處理...")
                audio = pydub.AudioSegment.from_file(audio_path)
                chunk_duration = 300 * 1000  # 5分鐘
                chunks = []
                total_chunks = len(audio) // chunk_duration + 1
